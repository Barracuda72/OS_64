...
char *IDT_addr;
char *IDT_reg;// = (char *)((unsigned long)IDT_addr + 256*8);
//extern unsigned long IDT_rsrvd;
...
void intr_install(unsigned char vector, void (*func)(), unsigned char type, /*unsigned char pcount,*/unsigned char ist, unsigned short selector)
{
 	unsigned char i;
 	unsigned char b[16];	//64-bit, да!
	...
	b[3] = selector>>8;	//кода
	//b[4] = pcount&0x0F;	//Количество аргументов обработчика - для 32-х разрядных систем
...
void intr_init()
{
	//IDT_addr = (char *)alloc_phys_page();	//TODO : SETUP!
	IDT_addr = (char *)(&IDT[0]);
